...
class UserProfile(models.Model):
    """
    Stores user-specific profile settings, preferences, and avatar,
    extending the base Account model.
    """
    ...
    avatar = models.ImageField(
        ...
        help_text=_('User profile picture or avatar.'),
    )
    post_anonymously = models.BooleanField(
        _('post_anonymously'),
        default=False,  # Default to showing username
        help_text=_("If checked, your username will be hidden in public community posts, showing 'Anonymous' instead."),

    )
    default_challenge_length = models.PositiveIntegerField(
        _('default_challenge_length'),
        null=True,
        blank=True,
        choices=[
            (7, '7 Days'),
        ...
        help_text=_('Your preferred default duration (in days) when creating a new challenge.'),
    )

    enable_global_reminders = models.BooleanField(
        _('enable daily check-in notifications'),
        default=True,
        help_text=_('Master setting to enable or disable all daily check-in email reminders.'),
    )

    created_at = models.DateTimeField(_("profile created"), auto_now_add=True)
    updated_at = models.DateTimeField(_("profile updated"), auto_now=True)
    ...
    def __str__(self):
        # Access email via the user field
        return f"{self.user.email}'s Profile"



# signal to auto-create/update Userprofile
@receiver(post_save, sender=settings.AUTH_USER_MODEL)
def create_or_update_user_profile(sender, instance, created, **kwargs):
    """
    Creates a UserProfile when a new Account is created.
    Ensures the profile is saved whenever the Account is saved.
    """
    if created:
        UserProfile.objects.create(user=instance)
    # Ensure the profile is saved even on updates to the Account model.
    # This helps if the profile somehow didn't get created initially
    # Using the user as primary key, avoids race conditions here vs get_or_create
