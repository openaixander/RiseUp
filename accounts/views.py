...
from challenge.utils import calculate_current_streak
from .models import Account, UserProfile
from challenge.models import DailyLog, UserAchievement, TimelineEvent, Reflection, Challenge
from .forms import RegistrationForm, UserProfileForm, AccountDeleteForm
...
def profile_settings_view(request):
    ...
    # check whether if the request method is a 'POST' request
    if request.method == 'POST':
        # only handle Userprofileform submission here now
        ...
        if profile_form.is_valid():
            try:
                ...
                logger.error(f"Error saving profile form for {user.email}: {e}", exc_info=True)
    ...
    # We still need the delete form for the modal context
...
def reset_progress_view(request):
    """
    Permanently deletes the user's account after email and password confirmation.
    Triggered by the form submission inside the delete confirmation modal.
    """
    ...
    if not password or not user.check_password(password):
        messages.error(request, "Incorrect password. Progress was not reset.")
        return redirect('accounts:profile_settings')
    
    try:
        logger.warning(f"Resetting progress for user {user.email}")
        # delete related data
        ...
        logger.info(f"Progress reset successfully for user {user.email}")
        messages.success(request, "Your progress has been completely reset.")
    
    except Exception as e:
        logger.error(f"Error resetting progress for user {user.email}: {e}", exc_info=True)
        logger.error(f"Error resetting progress for user {user.email}: {e}", exc_info=True)
        messages.error(request, "An error occurred while resetting your progress.")
    return redirect('accounts:profile_settings')
...
def delete_account_view(request):
    ...
    if form.is_valid():
        ...
        try:        
            ...
            # log the user which is very important
            auth_logout(request)
            user.delete()
            logger.warning(f"Account permanently deleted for former user: {user_email_for_log}")

            # redirect to homepage
            return redirect('riseup:index')
        except Exception as e:
            logger.error(f"Error during account deletion for former user {user_email_for_log}: {e}", exc_info=True)
            return redirect('riseup:index')
    else:
        # form validation failed (email mismatch or password incorrect)
        # Add form errors to message framework
        for field, errors in form.errors.items():
            for error in errors:
                # make the error user-friendly
                error_message = f"{error}" if field == '__all__' else f"{form.fields[field].label}: {error}"
                messages.error(request, error_message)
        # redirect back to the settings page where messages will show
        return redirect('accounts:profile_settings')